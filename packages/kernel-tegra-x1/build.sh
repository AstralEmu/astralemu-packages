 ...
 * the downstream kernel references from fork.c, sched/core.c, etc. but
 * are not needed for the Switch L4T build. All quadd_* functions are
 * safe as no-ops — the profiler hardware is not used. */
