#!/bin/bash
# kernel-arm64-modern-image — extract prep tarball, build Image + DTBs.
set -euo pipefail
...
mkdir -p "$OUT"
cp arch/arm64/boot/Image  "$OUT/vmlinuz-${KVER}-arm64-modern"
cp System.map             "$OUT/System.map-${KVER}-arm64-modern"
cp .config                "$OUT/config-${KVER}-arm64-modern"
...
echo "Image:  $(stat -c%s "$OUT/vmlinuz-${KVER}-arm64-modern") bytes"
