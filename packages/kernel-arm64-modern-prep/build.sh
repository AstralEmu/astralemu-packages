#!/bin/bash
# kernel-arm64-modern-prep — fetch Linux source, apply BORE + CachyOS
...
# packages/kernel-arm64-modern/patches/soc-downstream/<SoC>/ by
...
SOC_PATCHES=/workspace/packages/kernel-arm64-modern/patches/soc-downstream
...
if [[ -f /workspace/packages/kernel-arm64-modern/config/defconfig.fragment ]]; then
  scripts/kconfig/merge_config.sh -m -O . .config \
    /workspace/packages/kernel-arm64-modern/config/defconfig.fragment
