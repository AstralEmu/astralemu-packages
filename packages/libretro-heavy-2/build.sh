#!/bin/bash
set -e
export PATH="/usr/lib/ccache:$PATH"
export CCACHE_MAXSIZE=5G
ccache -z

export CFLAGS="$DEVICE_CFLAGS -flto=auto"
export CXXFLAGS="$DEVICE_CXXFLAGS -flto=auto"
export LDFLAGS="-flto=auto -shared -ljemalloc"

CORES_DIR=/workspace/libretro-cores
PKG_DIR=/workspace/cores-heavy-2
mkdir -p "$CORES_DIR" "$PKG_DIR"
cd "$CORES_DIR"

build_core() {
  local repo=$1
  local name=$2
  local subdir=${3:-}
  local make_args=${4:-}
  local pre_make=${5:-}
  local post_clean=${6:-}

  echo "=== Building $name ==="
  if [[ -d "$name" ]] && [[ -f "$name/.gitmodules" ]] && [[ -z "$(ls -A "$name/libretro-common" 2>/dev/null)" ]]; then
    rm -rf "$name"
  fi
  if [[ ! -d "$name" ]]; then
    if ! git clone --depth 1 --recursive "https://github.com/libretro/$repo.git" "$name"; then
      echo "ERROR: Failed to clone $repo, skipping $name..."
      return 1
    fi
  fi
  cd "$name"
  [[ -n "$pre_make" ]] && eval "$pre_make"
  [[ -n "$subdir" ]] && cd "$subdir"
  make clean 2>/dev/null || true
  [[ -n "$post_clean" ]] && eval "$post_clean"
  timeout ${BUILD_TIMEOUT}s make -j$(nproc) platform=unix $make_args \
    CC="ccache gcc" CXX="ccache g++" \
    LDFLAGS="$LDFLAGS" SKIPDEPEND=1 WERROR=0 || {
    EXIT_CODE=$?
    [[ $EXIT_CODE -eq 124 ]] && echo "TIMEOUT on $name" && exit 124
    exit $EXIT_CODE
  }
  find . -name "*.so" -exec cp {} "$PKG_DIR/" \;
  cd "$CORES_DIR"
}

# Batch 2: Flycast, ScummVM
build_core flycast flycast "" "HAVE_GENERIC_JIT=0"
# scummvm on x86: disable libco inline asm (LTO + RIP-relative TLS = wrong relocations)
# make clean deletes deps/, so restore via git submodule update (reliable, unlike make -n),
# then sed patches libco before the real build. configure_submodules.sh during make is a
# no-op since the submodule is already at the recorded commit (won't overwrite our patch).
build_core scummvm scummvm "backends/platform/libretro" "" "" \
  '[[ "$DEVICE_ARCH" == "amd64" ]] && { (cd "$CORES_DIR/scummvm" && git submodule update --init --recursive); sed -i "/#define CO_USE_INLINE_ASM/d" deps/libretro-common/libco/amd64.c; } || true'

ccache -s
echo "completed" > /workspace/build-status-heavy-2
